...
| Phase | Tool | Purpose |
|---|---|---|
| Design | `${PLUGIN_ROOT}/bin/patch-knowledge-document` | Add features and constraints to `spec.k.json` |
| Validate | `${PLUGIN_ROOT}/bin/check` | Verify constraints fail on empty codebase |
| Implement | Direct code edits | Build feature to satisfy constraints |
| Verify | `${PLUGIN_ROOT}/bin/check` | Confirm all constraints PASS |
...
```bash
${PLUGIN_ROOT}/bin/patch-knowledge-document spec.k.json '[
  {
    "op": "add",
...
## CRITICAL: Use `bin/check`

**ALWAYS invoke `${PLUGIN_ROOT}/bin/check`.** This is the stable
interface for constraint checks.
...
**Default path resolution (when no positional arg is given):**
1. `$PROJECT_ROOT/project.k.json` — if present, check every spec it lists (each with its own envs + per-spec `PROJECT_ROOT`).
2. `$PROJECT_ROOT/spec.k.json` — otherwise.

**Arguments:**
- Positional — Path to a `Project` (`project.k.json`) or a `Spec` (`spec.k.json`).
- `--features` — Comma-separated feature IDs (spec mode only; ignored in project mode).
- `--output-checks-path` — Where to save ChecksResults (spec mode only; defaults to `spec-checks.k.json` next to the spec).
- `--dry-run` — Skip execution; load existing ChecksResults file and print the previous run's report.
- `--full-report` — Verbose output. Without it: on total success the output is just `PASS`; on failure or unverified, only the relevant warning/error sections print.
...
- **NEVER** edit directly
- **ALWAYS** use `${PLUGIN_ROOT}/bin/patch-knowledge-document`

## Constraint Execution
- **NEVER** manually run constraint commands for validation
- **ALWAYS** invoke `${PLUGIN_ROOT}/bin/check`
...
- **Constraint checker**: `${PLUGIN_ROOT}/bin/check`
- **Knowledge patcher**: `${PLUGIN_ROOT}/bin/patch-knowledge-document`
...
- Recursive execution of the checker within constraints is detected and prevented
