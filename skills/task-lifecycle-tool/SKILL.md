...
When coding agents complete work and add a new iteration to task.json:

1. **Run feature constraint checks** using the features-checks-tool:
   ```bash
   python constraints_tool/constraints_tool/task_features_checker.py task.json --output-checks-path checks_results.json
   ```
