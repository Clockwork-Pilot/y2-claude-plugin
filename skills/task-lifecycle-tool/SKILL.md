...
### Update task
Update `task-iterations.k.json` by applying a JSON Patch (RFC 6902). This will update `task-iterations.k.json` and automatically regenerate `task-iterations.k.md`.

```bash
python ${CLAUDE_PLUGIN_ROOT}/knowledge_tool/knowledge_tool/patch_knowledge_document.py task-iterations.k.json '[{"op": "replace", "path": "/label", "value": "New Label"}]'
```
...
**Limitations:** 
- `check_spec_constraints.py` API Script checks features constraints, and should not be used directly in constraints' `cmd`. Attempt to call it recursively will immediately fail.
...
1. **Run feature constraint checks** using the check_constraints:
   ```bash
   python constraints_tool/constraints_tool/check_spec_constraints.py task-iterations.k.json --output-checks-path task-results.k.json
   ```

2. **Extract features_stats** from the ChecksResults output (automatically generated by check_spec_constraints.py)
